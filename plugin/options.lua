...
package_symbols_finder = function(opts, ctx)
  ...
  if not vim.api.nvim_buf_is_loaded(buf) then
    ...
    vim.defer_fn(function() vim.api.nvim_del_autocmd(id) end, 10000)
    return function() ctx.async:sleep(2000) end
  end
  ...
  return function(cb)
    M.request(buf, method, function() return p end, function(client, result, params)
      ...
      local items = M.results_to_items(client, symbols, {
        ...
        filter = function(item) return want(M.symbol_kind(item.kind)) end,
      })
